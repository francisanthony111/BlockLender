...
contract CrowdBank {
     ...
     function acceptProposal(uint proposeId) public
    {
     ...
     function getProposalAtPosFor(address lender, uint pos) public view returns(address, uint, ProposalState, uint, uint, uint, uint) {
        Proposal storage prop = proposalList[lendMap[lender][pos]];
        return (prop.lender, prop.loanId, prop.state, prop.rate, prop.amount, loanList[prop.loanId].amount, loanList[prop.loanId].dueDate);
    }
...
// BORROWER ACTIONS AVAILABLE    
     ...
     function getLoanDetailsByAddressPosition(address borrower, uint pos) public view returns(LoanState, uint, uint, uint, uint) {
